...
你需要一个流动性仓位用来质押到对应的农场。对应的农场只能质押来自对应的代币对的流动性仓位，并要选择确切的费用级别；例如，CAKE-BNB 0.25%农场只接受 0.25% 费用级别的 CAKE-BNB 流动性仓位质押。它不接受：&#x20;

* 其他代币对的流动性仓位，如 CAKE-BUSD&#x20;
* 相同的代币对的流动性仓位，但是属于其他费用级别：如费用级别为 0.05% 的 CAKE-BNB 流动性仓位 。&#x20;

要创建准确的 LP 仓位，你需要为该交易对提供流动性，并选择对应农场的费率级别。因此，要获得 CAKE-BNB 0.25%的 LP 仓位，你首先要为 CAKE-BNB 交易对提供流动性，并选择 0.25% 的费用级别。 这看起来似乎有点麻烦，实际上非常简单。以下是分步教程。
...
你可以选择另一种排序选项，如按年利率排序，查看目前利率最高的农场。请注意，全局年化利率使用有效和质押的流动性总量与糖浆池 CAKE 奖励分发量计算得出。各个持仓的年化利率可能会有所不同，具体取决于其价格范围设置。

当你找到一个你想参与的农场时，记下交易对和它的费用率，例如 BNB-CAKE 和 0.25%，用于之后添加对应的流动性。

### 提供流动性以建立仓位&#x20;

现在你已经找到了准备参与的农场，现在你需要添加流动性：&#x20;

1 - 如果你没有任何可用的流动性仓位，你会看到一个 "增加流动性 "的按钮，只需点击它就可以打开 "增加流动性 "窗口，不需要离开农场页面。
...
2 - 或者你也可以点击农场下方的详情或者展开按钮。将显示更多细节，点击详情部分中的 "添加XXX-YYY LP "链接来添加流动性。
...
你可以通过重复上述步骤，快速为不同价格区间配置的多个仓位进行质押。每个仓位都会赚取 CAKE，需要分别被收割。

### 收割农场奖励&#x20;

在 V3 农场中，你可以在同一个农场中质押多个流动性仓位。每个仓位都会获得CAKE，需要分别被收割。
...
你将需要一些 “LP 代币” 才能使用农场。对应农场只接受自己对应的 LP 代币（LP token）； 例如，CAKE-BNB 农场将只接受 CAKE-BNB LP 代币。
