...
这是由于 V2 交易系统遗留的一个 bug（存在于每个 UniSwap V2 forks 中），如果一个代币对是以下情况，将无法透過 PancakeSwap 流动性添加界面、或者通过调用合约来为该代币添加流动性：&#x20;

* 在 FactoryV2 上调用 `createPair`  创建，但未存入初始流动性和铸造初始 LP 代币&#x20;
* 然后，手动将代币对中其中一个的代币转入了流动性池合约中，并调用了`sync`

{% hint style="info" %}
最近在 BNB 链上的 PancakeSwap  V2 上观察到越来越多的此类攻击。&#x20;
...
如果您的代币对受到此类攻击影响，您将会看到一个错误提示弹窗，并提醒您 BscScan 上该代币对/流动性池页面链接。
...
在 BscScan 页面中的代币余额区域，查看哪个代币已存入池中。通常，已转入的代币是代币对中的配对代币。（如 WBNB、USDT 等......）&#x20;

然后，您必须手动将缺失的另一种代币转入流动性池合约地址。您可以透过在任意钱包应用程序中，输入流动性池地址作为接收方即可。&#x20;
...
* `tokenToSend.price`：即将转入到池中的代币的美元价格（发行价格）&#x20;
* `pool`：v2 池
