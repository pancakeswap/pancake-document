...
{% hint style="warning" %}
* 迁移是**单向的**、**一次性的**，包含你 CAKE pool 仓位中当前所有的 CAKE（包括奖励）。不支持部分委托。
* 迁移只能在剩余锁定时间超过 7 天的有效仓位上执行。
* 你**不能**为已迁移的仓位延长或添加更多 CAKE。但是，你可以通过锁定 CAKE 创建一个新的 veCAKE 仓位。然后，在原生 veCAKE 质押仓位上执行添加 CAKE 或延长仓位的操作。
{% endhint %}
...
如果你在 CAKE pool 中持有一个有效的固定期限 CAKE 质押仓位，你应该会看到迁移选项。

只需点击该按钮，并在钱包弹窗中确认交易即可。
...
然而，已迁移的仓位无法更新。如果你想获得更多 veCAKE，请通过锁定额外的 CAKE 在右侧创建一个新的原生 veCAKE 质押仓位。&#x20;

创建原生仓位后，你可以使用右侧的面板开始延长其锁定时长，以及添加更多 CAKE。
...
或者，你可以点击"View CAKE Pool Position"前往 CAKE pool 查看更多详情。

## 从 CAKE 质押池提取 CAKE <a href="#id-069e3aca-6b5d-45b0-a07c-ef4645ee0ccd" id="id-069e3aca-6b5d-45b0-a07c-ef4645ee0ccd"></a>

已迁移的仓位将根据其原始结束时间解锁。解锁后，前往 Pools 页面提取已质押的 CAKE；将它们添加到 veCAKE 质押中，以提升你的 veCAKE 余额以及其他丰厚的好处。

请注意，由于销毁机制仍然有效，为避免奖励被销毁，请在迁移仓位解锁时尽快提取 CAKE 并将它们添加到 veCAKE 中。
