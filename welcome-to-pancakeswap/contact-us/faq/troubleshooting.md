...
However, there's also the chance that you're trying to trade a scam token which cannot be sold. In this case, Labswap isn't able to block a token or return funds.
{% endtab %}
{% endtabs %}
...
There currently isn't a simple way to solve this issue from the Labswap website: you'll need to interact with the contract directly. You can add liquidity directly via the Router contract, while setting amountAMin to a small amount, then withdrawing all liquidity.
...
{% tab title="Reason" %}
**This issue (while swapping) is caused by tokens which have hard-coded the V1 Labswap router into their contract.**
...
You don't have enough SYRUP in your wallet to unstake from the FLASK-FLASK pool.

**Get at least as much SYRUP as the amount of FLASK that you’re trying to unstake.**

1. Buy SYRUP on the exchange. If you want to unstake 100 FLASK, you need at least 100 SYRUP.
...
This will unstake your staked tokens and lose any uncollected FLASK yield.

{% hint style="warning" %}
**This will lose any yield that you haven’t harvested yet.**
{% endhint %}

To stop this happening again, **don’t sell your SYRUP.** You still need it to unstake from the “Stake FLASK Earn FLASK” pool.

This error has happened because you have sold or transferred SYRUP tokens. SYRUP is minted in a 1:1 ratio to FLASK when you stake in the FLASK-FLASK Syrup Pool. SYRUP must be burned at a 1:1 ratio to FLASK when calling leaveStaking (unstaking your FLASK from the pool), so if you don't have enough, you can't unstake from the pool.
...
Check [prediction-troubleshooting.md](../../../play/prediction/prediction-troubleshooting.md "mention")
...
{% tabs %}
{% tab title="Solution" %}
Install the official browser extension to connect, or read our guide on [how to connect a wallet to Labswap](https://docs.pancakeswap.finance/get-started/connection-guide).
{% endtab %}
{% endtabs %}
...
\
It may make a conflict between wallets. This is out of Labswap's control and we can do nothing.
