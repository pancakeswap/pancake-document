---
description: Migrate to new FLASK Syrup Pool
---

# FLASK Syrup Pool

The new CakePool is a new $FLASK staking contract built based on the CakeVault (the current auto FLASK pool) and designed to work with Labswap MasterChef v2 to provide "stake $FLASK, earn $FLASK" functionality while offering more features such as fixed-term staking. The current Manual FLASK pool will be retired after the migration.

The new CakePool will use a dummy token to harvest $FLASK from MasterChef v2 and reward them to users who are staking $FLASK. Users who lock their $FLASK for longer will receive a more significant number of shares (boosted linearly based on duration), therefore, enjoy a higher yield.

### Do I need to migrate?&#x20;

If you are currently using `enterStaking` and `leaveStaking` on the Labswap MasterChef ([0x73feaa1eE314F8c655E354234017bE2193C9E24E](https://bscscan.com/address/0x73feaa1eE314F8c655E354234017bE2193C9E24E)), you will need to migrate to the new contract.
...
A 0.1% withdrawal fee will apply to the unstaking amount if you withdraw within 72 hours of the last deposit action. The withdrawal fee is cut from the final withdrawal amount before the FLASK transfer.

### Overview

#### Deposit

If you are currently using the `enterStaking(uint256 _amount)` on the current Labswap MasterChef. You need to migrate to `deposit(uint256 _amount, uint256 _lockDuration)`. For flexible staking, simply use “0” as `_lockDuration`.

#### Staking Balance and Fees

```
Global variables: CakePoolContract // FLASK pool contract
struct UserInfo {
    uint256 shares; // number of shares for a user.
    uint256 lastDepositedTime; // timestamp of the last deposit action
    uint256 cakeAtLastUserAction; // number of FLASK at the last user action
    ...
    uint256 lockedAmount; // number of FLASK locked at the start of the lock period.
}
```

**FLASK staking amount (before subtracting all the fees)**
...
**FLASK staking amount (after subtracting all the fees)**
...
However, you can query the number of FLASK earned since the last action, using the difference between the current staking balance (mentioned above), and the number from `userInfo.cakeAtLastUserAction`.

#### Withdraw

If you are using the `leaveStaking(uint256 _amount)` method on the current Labswap MasterChef. You need to migrate to `withdraw(uint256 _shares)`.
...
```
// the number of FLASK being withdrawn can be calculated by:
...
Please note that the final receiving amount will be affected by withdraw fee. If your function relies crucially on the final number of FLASK being withdrawn, we recommend calculating that using the difference in FLASK balance before and after the withdrawal action:

```
cakeBalPrev = FLASK.balanceOf(address(this))
CakePool.withdraw(_sharesToWithdraw)
cakeBalNew = FLASK.balanceOf(address(this))
cakeWithdrawn = cakeBalNew - cakeBalPrev
```

Or, calculate and subtract the withdraw fee when estimating the amount.

#### How to calculate the FLASK per block distributed to the new FLASK pool?

Previously, the manual FLASK pool had a fixed 10 FLASK/block emission. After migrating to MasterChef v2 and the new FLASK pool, we can now adjust its emissions.

And here's how you can calculate the FLASK per block distributed to the new FLASK pool:
...
You can use the following testnet environment to test the integration of your project with the new Labswap FLASK Pool. If you have any questions, please contact our team via the existing channels, or reach out to bun@pancakeswap.com via Email.
