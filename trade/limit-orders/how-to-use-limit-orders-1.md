...
选择一个代币对（例如 BNB/CAKE）以及您想要卖出 / 买入的金额
...
| 状态             | 描述                                                                                     |
| ---------------- | ---------------------------------------------------------------------------------------- |
| 待处理 (Pending) | 等待价格达到您的目标                                                                      |
| 已成交 (Filled)  | 订单已执行，资金已发送到您的钱包                                                          |
| 部分成交 (Partially Filled) | 您的订单仅部分执行。您将同时持有两种代币（例如，一部分 BNB，一部分 USDT）       |
| 已取消 (Cancelled) | 您已取消订单。您的所有资金均已退还给您                                                  |

### 常见问题解答

**问：我需要支付手续费来下达限价单吗？**

答：不需要。相反，当您的订单执行时，您会赚取 0.1% 的交易手续费。
...
答：您将同时持有两种代币。您可以随时取消并提取两种代币以及赚取的手续费。

**问：我的订单已成交，但我的钱包中尚未收到资金？**

答：在极少数情况下可能会发生这种情况，但您的资金始终是安全的。只需使用订单详情 UI 中的"Withdraw"按钮即可手动领取资金。
