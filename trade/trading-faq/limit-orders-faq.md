...
限价单在达到所需价格时执行，但是，由于 gas 波动，实际执行价格可能与您在界面上指定的价格有所不同。通常，执行价格和所需价格应该几乎相同，但是，如果您提交了一个特别小的订单（\~<1000$），执行价格可能会略高，以计入手续费。&#x20;
...
滑点与限价单无关。您指定输入金额（例如 1000 CAKE）和输出金额（例如 20 BNB），如果该交易对的价格达到所需价格，限价单保证您将以您的输入金额（1000 CAKE）收到不少于指定的输出金额（20 BNB）。**请注意，转账征税代币不应与限价单一起使用**（见上文）

### 实际执行价格显示"never executes"。这是什么意思？

这基本上意味着您试图兑换非常少量的代币，因此没有足够的代币来计入 gas 费。一般来说，您需要增加"input"字段的金额以消除此错误。&#x20;
...
要以低于市场价格卖出，您需要**止损限价单 (Stop Limit Orders)**，而不是限价单。止损限价单功能即将推出。
