...
你可以从[网站](https://pancakeswap.finance)进入，在 Trade → Perpetual 部分，或通过此[链接](https://perp.pancakeswap.finance/en/futures/BTCUSDT)。

### 存入资金

你需要先将资金存入期货账户，操作位于屏幕右下角的方框中。
...
可以使用 USDT、BUSD、APX 或 CAKE（有奖励！）作为抵押品来支付交易费用。
...
> 注意：交易费用为名义价值的 0.02%（挂单方/maker）和 0.07%（吃单方/taker）。CAKE 将是交易费用的默认支付选项，其次是 APX（ApolloX 的代币）和 USDT。任何使用 CAKE 支付交易费用的用户都将享受 5% 的交易费用折扣。
...
是时候交易了！让我们认真起来。你可以使用所有可用的工具，按自己的方式分析图表。
...
免责声明：请记住，高杠杆水平蕴含着非常高的风险，请明智地使用。
...
> 注意：还有一些其他选项需要考虑，你可以在[交易术语表](perpetuals-glossary.md)中查看。<br>
...
订单提交后，它会显示在 "Open Orders" 中，直到成交为止。
...
成交后，你的仓位将处于活跃状态。你可以在仓位面板中查看它。你还可以查看、编辑或关闭它。
