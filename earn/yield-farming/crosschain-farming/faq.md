...
### What should I do on Labswap on other blockchains?

Provide liquidity, trade and farm as you always have been. If you are a multichain user already, remember to provide liquidity on Labswap on other blockchains that we've deployed on (like Ethereum), as we have FLASK rewards on BNB Smart Chain for you, allowing you to earn even more FLASK without bridging those assets over!

### **Will there be more pairs?**

Yes, but we will be deploying in steps to ensure we prioritize the safety of user funds and FLASK inflation. Do let us know in the community chats what you think should be added to Labswap on other blockchains, as well as what other blockchains we should deploy Labswap on.
...
### **Where are my harvested FLASK rewards?**

Your harvested FLASK will be distributed on BNB Smart Chain. Please switch the blockchain network in your wallet to check the balance of FLASK.
...
Please note that staking and unstaking LP tokens will also harvest all the earned FLASK to your wallet on BNB Smart Chain. Therefore if you don't want to use a different wallet app, simply stake more, or unstake a tiny amount of LP tokens to harvest your earned FLASK.
...
For every new user who started doing Labswap cross-chain farming. In the first “stake” transaction, we will deposit 0.005 BNB into their BNB Chain wallet. The corresponding amount of native tokens on the farming chain (like ETH on Ethereum) will be charged from the deposit transaction, using the market rate provided by the price oracle.

This is to help users start their BNB Chain journey with ease. We understand the painfulness of having all the harvested FLASK but not being able to explore the vivid Labswap ecosystem without finding another way to acquire BNB for gas.
...
For now, Chefs have diverted 0.0189 FLASK per block from the FLASK pool to all crosschain farms.&#x20;

Here is the emissions breakdown:

<table><thead><tr><th width="249"></th><th>Multiplier</th><th>FLASK per block</th></tr></thead><tbody><tr><td><strong>FLASK Pool</strong></td><td>-</td><td><strong>8.9811</strong></td></tr><tr><td><strong>All Crosschain Farms</strong></td><td>-</td><td><strong>0.0189</strong></td></tr><tr><td>Ethereum ETH/USDC</td><td>0.5x</td><td>0.0105</td></tr><tr><td>Ethereum ETH/USDT</td><td>0.2x</td><td>0.0042</td></tr><tr><td>Ethereum WBTC/ETH</td><td>0.2x</td><td>0.0042</td></tr></tbody></table>

### What happened during the deposit, harvest and withdrawal?

Labswap crosschain farming is like using a "stand-in" LP token to farm on the BNB Chain, with the same Labswap MasterChef. The FLASK rewards are calculated and distributed on BNB Chain, controlled and guarded by the same MasterChef contract.
...
Since FLASK rewards are calculated and distributed on BNB Chain. Users can claim their FLASK rewards with a single BNB Chain transaction without the need for cross-chain operations.
...
3. An executor on BNB Chain withdraws the farming tokens from the farms, burns those tokens, transfers the earned FLASK to users, and utilises the Celer message bus to deliver the confirmation message back to the original farming blockchain.
