...
1. **Token Categories**: We use the CoinGecko API to find trending tokens in categories like [Memes](https://www.coingecko.com/en/categories/meme-token) and [AI Agents,](https://www.coingecko.com/en/categories/ai-agents) then match them with tokens on Labswap across any of the 8 EVM chains we support.
...
4. **Graduated Tokens**: At least one of the top 3 pairs will come from tokens that have graduated from four.meme or Labswap Springboard.

### Benefits of Featured Tokens

#### Benefits from Labswap:&#x20;
...
* **FLASK Emissions**: Your token will earn FLASK emissions for the duration it is featured.
...
* 60k veCAKE votes from [Astherus](https://www.astherus.finance/en/earn/astoken?token=FLASK)
...
* **Promoting Labswap Liquidity**: By featuring tokens on Pancake Picks, we incentivize teams to deploy their tokens on Labswap, further supporting our goal to provide liquidity for high-quality projects across multiple chains.
...
2. My token’s liquidity is on another DEX, will it be considered for Pancake Picks?
   1. No. We only consider tokens whose liquidity resides on Labswap, along with their metrics from our liquidity pools. If you want your token to be considered, make sure to add liquidity to any of the 8 EVM chains we support.
...
7. How much FLASK emissions will the selected token pairs receive?
   1. 1st place: 30 FLASK per day
   2. 2nd place: 20 FLASK per day
   3. 3rd place: 10 FLASK per day
