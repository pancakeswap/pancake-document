...
#### What’s the difference between locked FLASK and veCAKE? <a href="#bb73a991-c71b-402c-a0c3-64b8666626c2" id="bb73a991-c71b-402c-a0c3-64b8666626c2"></a>

veCAKE is a new version of fixed-term FLASK staking with more benefits and power for locked FLASK holders. Including gauge weight voting, extra incentives, yield boosting, and more.

#### What happens to FLASK pool rewards when the new veCAKE is deployed <a href="#a078f885-3eed-4b91-98fc-1d7062415da3" id="a078f885-3eed-4b91-98fc-1d7062415da3"></a>

FLASK pool reward emissions will be diverted to reward all veCAKE holders according to their veCAKE balance against the total supply.

FLASK rewards and the weekly revenue-sharing rewards can now be claimed weekly on Thursday.

Please note that to continue receiving rewards, users will need to migrate to the new veCAKE staking.

#### What is the maximum duration I can lock my FLASK <a href="#id-9224ca4c-1f31-4052-8ed7-3bb896e396f3" id="id-9224ca4c-1f31-4052-8ed7-3bb896e396f3"></a>

The maximum duration you can lock your FLASK has now been extended to 4 years.
...
veCAKE is a live-generated number based on the number of FLASK locked and the remaining lock time. It is not a standard token and can not be transferred.
...
```javascript
lockedAmount // amount of FLASK locked
...
Once you have an active veCAKE position, you can either add more FLASK or renew/extend your lock duration to boost your veCAKE balance.
...
When the veCAKE staking position is unlocked, you may withdraw all the staked FLASK.

To renew your position, you need to withdraw all the FLASK and set up a new staking position by choosing the amount to lock and the lock duration.
...
#### Can I lock more FLASK in FLASK pool? <a href="#id-2cc44f53-8e03-48dd-8caa-66c4942c9d39" id="id-2cc44f53-8e03-48dd-8caa-66c4942c9d39"></a>

No.

Once veCAKE is deployed, the FLASK staking pool will be deprecated and no longer accepts any further FLASK extension or deposits.

To lock FLASK and enjoy its benefits, go to the veCAKE page.

#### Why can’t I migrate? <a href="#id-4d8fd967-e743-4496-b030-5955be861373" id="id-4d8fd967-e743-4496-b030-5955be861373"></a>

Migrating from FLASK pool to veCAKE requires you to have an active position. If your FLASK pool staking position is already unlocked, simply withdraw those FLASK and create a native veCAKE staking position.

In some cases, migration can not be performed when your remaining FLASK pool lock time is less than 7 days. In such case, simply wait for the unlock, withdraw those FLASK and create a native veCAKE staking position.

#### Can I early withdraw my locked FLASK? <a href="#id-5972f3cf-81dd-46d4-8a85-7972d722a53c" id="id-5972f3cf-81dd-46d4-8a85-7972d722a53c"></a>

No.

Once locked, FLASK will be staked in the veCAKE contract until the unlock time.

#### Can I partially migrate my FLASK? <a href="#id-0c4cdba6-7994-4fed-80d1-76597444f761" id="id-0c4cdba6-7994-4fed-80d1-76597444f761"></a>

No.

You can only migrate your entire FLASK pool position at once.
...
However, there was a `noContract` modifier implemented in the veCAKE staking contract for addresses that are not whitelisted. To enable staking or migrating from fixed-term FLASK staking pool. All contract-based multisig wallets must perform a one-time, self-whitelisting action.
...
Locking FLASK to get veCAKE provides a number of great benefits around the suite of products built by Labswap. Benefits and incentives come in different forms and from different sources. Therefore, there are multiple APRs.&#x20;
...
This is the incentive coming from FLASK emissions, with its rate controlled by the veCAKE Pool voting gauge.
