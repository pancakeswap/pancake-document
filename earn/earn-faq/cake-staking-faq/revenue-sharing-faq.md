...
不需要，您只需注册一次。所有后续的 CAKE 质押池操作都将自动通知收入分成池，并在下一次每周分配时更新您的份额。
