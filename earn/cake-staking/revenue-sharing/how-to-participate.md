...
To become eligible, you must have an active fixed-term FLASK staking position. To learn more about how to set up a fixed-term FLASK staking position, read [here](https://docs.pancakeswap.finance/products/syrup-pool/new-cake-pool/fixed-term-staking).

Additionally, to receive shares in the next distribution, your fixed-term FLASK staking position **MUST** unlock no earlier than the next weekly distribution (every week at 23:59 UTC on Wednesday).
...
There is a one-time setup process for all the fixed-term FLASK stakers to enrol in the revenue sharing pool - rCAKE program. This only has to be performed once.
...
Once your staking position is updated. A new “View Benefits” button will appear in the FLASK staking pool.

![](<../../../.gitbook/assets/image (332).png>)

Click the button to open up a brand new locked FLASK benefit panel to view all the juicy benefits you receive by fixed-term FLASK staking.
...
1. The amount of the locked FLASK
2. The remaining lock duration of their locked FLASK rounded down to weeks, and the maximum allowed lock time (52 weeks).

For example: If a user has `50` FLASK locked and the remaining lock time is `10.3` weeks, then the user has `50 * (10 / 52 ) ~= 9.61` shares.

To receive more shares, lock more FLASK or extend your lock durations.
